...
select book.*, book_price / book_pagecount "페이지당 가격" from book 
order by "페이지당 가격" asc, book_no asc;

select book.*, book_price / book_pagecount 페이지당가격 from book 
order by 페이지당가격 asc, book_no asc;
...
select lecture.*, lecture_price / lecture_duration 시간당수강료 from lecture
order by 시간당수강료 asc, lecture_no asc;
